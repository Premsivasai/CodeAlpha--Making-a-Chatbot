...
import {
  ShoppingBag, Sparkles, Mail, Lock, ArrowLeft, ArrowRight,
  ShieldCheck, AlertCircle, RefreshCw, KeyRound, Chrome, Github
} from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOtp({
        email: email.trim(),
        options: {
          shouldCreateUser: false // Sign in only (don't create profile unless signing up)
        }
      });
      ...
      if (error) {
        ...
        toast({
          title: "OTP Passcode Sent!",
          description: "Check your email for the 6-digit login verification code (use 123456 in dev)."
        });
        setStep("otp");
        setResendTimer(60);
      }
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // Store profile name in localStorage if it was loaded for Goval's legacy layout check
  ...
  return (
    <div className="min-h-screen bg-[#050816] text-white flex flex-col md:flex-row relative font-sans overflow-hidden">
      ...
      {/* Aurora lights background decoration */}
      <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-[#7C3AED]/10 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-[#06B6D4]/10 rounded-full blur-[100px] pointer-events-none" />
        ...
        <div className="w-full max-w-md bg-white/[0.02] border border-white/5 p-8 rounded-3xl shadow-2xl backdrop-blur-xl relative">
          ...
          <div className="mb-8">
            ...
            <p className="text-xs text-white/40 mt-1.5">
              Enter your email to receive a passwordless OTP login link.
            </p>
          </div>
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs py-1">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="remember" className="text-white/50 select-none cursor-pointer">Remember Me</label>
                  </div>
                  <a href="#" onClick={(e) => { e.preventDefault(); toast({ title: "OTP link help", description: "Use email OTP flow to instantly recover/login." }); }} className="text-purple-400 hover:text-purple-300 transition-colors font-medium">Forgot Password?</a>
                </div>
